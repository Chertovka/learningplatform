...
from learningplatform.settings import ALLOWED_HOSTS
...
def send_new_comment_notification(comment):
    ...
    context = {'author': author, 'host': host, 'comment': comment, 'user': user}
    subject = render_to_string('email/new_comment_letter_subject.txt', context)
    body_text = render_to_string('email/new_comment_letter_body.html', context)
